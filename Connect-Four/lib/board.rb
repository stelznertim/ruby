class Board
  ...
  def valid_column?(input)
    return true if input >= 0 && input < columns && check_rows(input)
  ...
  def update(column, symbol)
    return 'You are not allowed to play two turns in a row' if symbol == @current_symbol

    @current_symbol = symbol
    grid.reverse.each_with_index do |row, row_index|
      next unless row[column].nil?

      grid[5 - row_index][column] = symbol
      break
    end
    show
    grid
  ...
  def game_over?
    grid.each do |row|
      return true if check_grid_line(row, @current_symbol)
    end

    grid.transpose.each do |column|
      return true if check_grid_line(column, @current_symbol)
    end

    diagonals = find_diagonals(grid)
    diagonals.each do |diagonal|
      return true if check_grid_line(diagonal, @current_symbol)
    end

    reverse_diagonals = find_diagonals(grid.reverse)
    reverse_diagonals.each do |reverse_diagonal|
      return true if check_grid_line(reverse_diagonal, @current_symbol)
    end
    false
  end
